...
typedef struct
{
    ...
    char name[20];
    int age;
}DATA; 	//DATA 是结构体的一个别名，那么在定义结构体的变量的时候 DATA data就可以； 
 
#include "2-4 ChainList.h"
#include "2-5 ChainList.c"
void ChainListAll(ChainListType *head) //遍历链表 
{
    ChainListType *h;   //定义了一个结构体指针变量
    DATA data;			//定义了一个DATA结构体变量，DATA结构体其实就是作为Node结构体的数据
    h=head;				//给结构体指针变量赋值，这个值是通过参数传递进来的，赋值其实就是赋地址值
    printf("链表所有数据如下：\n"); 
    while(h) //循环处理链表每个结点 
    {
        data=h->data;//获取结点数据 
...
int main()
{
    ChainListType *node, *head=NULL;			//初始化结构体指针变量(数据和指针)为空
    DATA data;
